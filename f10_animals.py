...
class Window(object):    
    ...
    def update(self):
        for i in range(len(self.animals)):
            ...
            self.pos[i] = self.pos[i] + (np.random.rand(1, 3) - np.ones(shape=(1,3)) * 0.5) * np.array([1, 1, 0])
    ...
    def eat(self):
        ...
        for i in range(len(self.animals)):
            for j in range(len(self.plantPos)):
                if (distance(self.plantPos[j], self.pos[i]) < 1 + 0.1 * self.size[i]):
